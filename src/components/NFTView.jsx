...
const NFTView = ({ wallet }) => {
    ...
    const getNfts = async () => {
        ...
        setLoading(true)
        try {
            let response = await axios.get(`https://eth-mainnet.g.alchemy.com/v2/PuaAv6KQwf4lquxAoZZUKnzn9-tw-Rug/getNFTs?owner=${wallet}&withMetadata=true`)
            ...
            while (true) {
                ...
                if (pageKey) {
                    response = await axios.get(
                        `https://eth-mainnet.g.alchemy.com/v2/PuaAv6KQwf4lquxAoZZUKnzn9-tw-Rug/getNFTs?owner=${wallet}&withMetadata=true&pageKey=${pageKey}`
                    );
                    data = response.data;
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 mx-auto">
                            {
                                ...
                                nfts.map((nft, i) => {
                                    return (
                                        <div key={i} className="flex flex-col justify-center items-center border border-gray-500 mx-2 rounded-lg shadow-md transform hover:scale-105 transition duration-500">
                                            {/* <img 
                                        src={nft?.metadata?.image} alt="nft_image" 
                                        onError={({ currentTarget }) => {
                                            currentTarget.onerror = null; // prevents looping
                                            currentTarget.src="https://media3.giphy.com/media/R1cZlHJ7lWLBM80L3z/giphy.gif?cid=790b76118f710e684188cf5ce02a666449a8fd5b6a94f657&rid=giphy.gif&ct=g";
                                        }} 
                                    /> */}
                                            <img
                                                src={
